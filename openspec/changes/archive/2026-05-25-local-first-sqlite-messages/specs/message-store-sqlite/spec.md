...
The database SHALL declare its schema version and apply forward-only migrations on app launch. Schema changes SHALL never destroy user data unless the migration explicitly opts in (with a UI confirmation for destructive migrations).
...
- **THEN** all rows in `messages`, `conversations`, and `sync_state` tables SHALL be deleted
