...
- **THEN** the QuoteBubble shows the display name and a label "📷 Hình ảnh" (or equivalent localized label)
...
- **THEN** the client does NOT crash; it shows a toast "Không tìm thấy tin nhắn gốc" or remains idle (behavior MAY be either; no loading-backward required for this change)
