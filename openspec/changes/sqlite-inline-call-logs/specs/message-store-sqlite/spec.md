...
The mobile app SHALL maintain an on-device SQLite database (`koola.db`) that is the canonical source of message, conversation, and call-log data for the chat UI. ChatScreen and ConversationListScreen SHALL read exclusively from this database on the hot path; the network layer is consulted only by the sync engine, never directly by UI code. The database holds messages, conversations, call logs, and sync state, with a typed repository API so UI code never writes raw SQL.

#### Scenario: ChatScreen reads from SQLite, not REST, on conversation open

- **GIVEN** conversation C has at least one message or call log persisted in the local database
- **WHEN** the user navigates to ChatScreen for C
- **THEN** the first React render SHALL receive messages produced by a SQLite query
- **AND** the first React render SHALL receive inline call logs produced by a SQLite query
- **AND** no `messagesApi.list` or `callLogsApi.getHistory` call SHALL be made by the mount effect before the first paint
- **AND** the loading indicator SHALL NOT be shown when cached data exists
...
The database SHALL declare its schema version and apply forward-only migrations on app launch. Schema changes SHALL never destroy user data unless the migration explicitly opts in (with a UI confirmation for destructive migrations). Schema version 2 introduces the `outbox` and `outbox_metrics` tables alongside the existing `messages`, `conversations`, `sync_state`, `account_state`, and `schema_version` tables; schema version 3 introduces the `call_logs` table and its indexes leaving existing v1/v2 tables and indexes unchanged.

#### Scenario: First launch creates the schema at the current version

- **GIVEN** no `koola.db` exists on disk
- **WHEN** the app launches
- **THEN** the database SHALL be created with the current schema version (now including `call_logs` when targeting v3)
...
- **WHEN** the app launches
- **THEN** migrations N+1, N+2, ..., N+k SHALL execute in order inside a single transaction per migration
...
#### Scenario: Migration v3 adds call_logs table without altering existing data

- **GIVEN** `koola.db` exists at schema version 1 or 2 with rows in `messages`, `conversations`, `sync_state`, and optionally `outbox`
- **WHEN** the app launches with build code targeting version 3
- **THEN** the migration SHALL create the `call_logs` table and its indexes via `CREATE TABLE IF NOT EXISTS` and `CREATE INDEX IF NOT EXISTS` statements
- **AND** the existing `messages`, `conversations`, `sync_state`, `account_state`, `outbox`, and `outbox_metrics` rows SHALL remain untouched
- **AND** `schema_version` SHALL be set to `3`
...
- **AND** the `call_logs` rows SHALL also be deleted
...
- **AND** user B SHALL see no call logs from user A
