...
Ensure mobile authentication screens (Login, Register, Forgot Password, OTP) remain fully accessible, keyboard-safe, and operable at supported text scales on both iOS and Android. Covers form scroll behavior, secure-input visibility controls, field-level validation feedback with assistive technology announcements, and WCAG AA contrast compliance.
...
- **NOTE** Display labels (the `label` prop) SHALL use full Vietnamese diacritics (e.g. "Mật khẩu", "Đăng nhập"). The `accessibilityLabel` MAY remain ASCII for screen-reader compatibility (e.g. "Mat khau").
