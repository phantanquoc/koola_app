# backend-perf-scale-readiness Specification

## Purpose
Backend performance hardening and scale-readiness: bounded hot-path queries with supporting indexes, no populate on `senderId`, parallel independent fetches in the send path, non-blocking image processing via `sharp`, global Redis-backed throttler quota, bounded Mongo pool with no runtime index builds, HTTP hardening middleware, gated production logging, index-aligned Moments feed, Redis-lock cron mutual exclusion, distributed receiver-only typing indicators, and a single shared Redis pool per instance. Created by archiving change perf-audit-fix-and-scale-ready.
