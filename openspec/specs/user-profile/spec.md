...
## Purpose
Defines the user profile data model, read/update operations for profile fields (bio, username, coverPhoto, dateOfBirth, gender), username availability checking, and the mobile EditProfile UI layout with edit sheets.
...
- **THEN** system returns HTTP 409 Conflict with message "Ten nguoi dung da duoc su dung"
...
- **THEN** system returns HTTP 400 with message "Ten nguoi dung khong duoc phep"
...
The mobile client SHALL render the `Chinh sua ho so` screen as a setting-row layout with three grouped sections (Thong tin co ban / Tai khoan / Ca nhan), where each row opens a dedicated bottom sheet to edit its field.
...
- **THEN** the screen shows:
  - Section "Thong tin co ban" with rows: Ten hien thi, Ten nguoi dung, Gioi thieu
  - Section "Tai khoan" with rows: Email (read-only), So dien thoai
  - Section "Ca nhan" with rows: Ngay sinh, Gioi tinh

#### Scenario: Row tap opens sheet
- **WHEN** the user taps any editable row
- **THEN** a bottom sheet opens with `Modal animationType="slide"` containing the relevant editor and a primary "Luu" action

#### Scenario: Email row read-only
- **WHEN** the user taps the Email row
- **THEN** no editor opens; the row displays a "Da xac thuc" badge and a copy-to-clipboard icon
...
- **THEN** the system displays a confirmation Alert "Bo thay doi?" before dismissing
...
- **THEN** the sheet shows an error indicator with message "Ten nguoi dung da duoc su dung" and disables the Save action
