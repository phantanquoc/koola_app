...
## Purpose
Provides per-message delivery state indicators on the mobile client so senders can see whether their messages are pending, sent, or read. Covers tick rendering, the message_read socket subscription, and the failed-send retry flow.
