...
Ensure mobile content presentation is consistent and user-friendly: locale-appropriate Vietnamese copy, understandable timestamps, actionable empty/error states, visible failure feedback for user-initiated operations, and resilient compact layout that accommodates varying content length.

## Requirements

### Requirement: Production mobile copy uses a consistent Vietnamese baseline
User-facing labels, headings, empty states, errors, and actions in production mobile surfaces SHALL use consistent Vietnamese terminology with full diacritics.

#### Scenario: Display copy uses diacritics
- **WHEN** Vietnamese text is rendered as a visible label, heading, button title, placeholder, or error message
- **THEN** it SHALL include correct Vietnamese diacritics (e.g. "Đăng nhập", not "Dang nhap"; "Mật khẩu", not "Mat khau")
- **AND** `accessibilityLabel` values MAY remain ASCII (diacritic-free) as an accessibility choice — this does not violate the display rule
