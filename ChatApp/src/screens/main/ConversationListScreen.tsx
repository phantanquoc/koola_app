  ...
  return (
    <SafeAreaView style={screenStyles.container} edges={['bottom']}>
      ...
      <Animated.FlatList
        ...
        // ListEmptyComponent only mounts when `data` is empty, so reaching the
        // final branches already means visibleConversations.length === 0. The
        // filter-specific state therefore keys off category alone (plus the
        // loading/offline guards that must keep their precedence).
        ListEmptyComponent={
          loading || refreshing ? (
            <KoolaLoadingState
